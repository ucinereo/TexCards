  ...
  addUserPermission(): void {
    ...
    if (permissionEntry) {
      if (!permissionEntry.writePermission && this.permissionType == "write") { // otherwise write permission already granted
        ...
        this.flashcardService.grantWritePermission(request).subscribe((response) => {
      ...
      if (this.permissionType == "read") {
        ...
        this.flashcardService.grantReadPermission(request).subscribe((response) => {
            const newEntry = new UserPermission(username, this.flashcardSetID, true, false);
            this.userPermissionList?.permissions.push(newEntry);
          });
      } else {
        const request = new GrantWritePermissionRequest(this.userName, this.flashcardSetID);
        this.flashcardService.grantWritePermission(request).subscribe((response) => {
          const newEntry = new UserPermission(username, this.flashcardSetID, true, true);
          this.userPermissionList?.permissions.push(newEntry);
        });
      }
    }
